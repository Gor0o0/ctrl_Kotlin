...
class SaveSystem(
    ...
    //////////////////////////////// <3.>
    fun listSavesForPlayer(playerId: String): List<File> {
        val dir = File("saves")
        if (!dir.exists()) return emptyList()

        return dir.listFiles()
            ?.filter { it.name.startsWith(playerId) && it.name.endsWith(".save") }
            ?: emptyList()
    }
    //////////////////////////////// </3.>
...
////////////////////////////////////////////////////////////////// <1.>
fun dealDamage(playerId: String, targetId: String, weapon: String, bus: EventBus) {
    val damage = when (weapon) {
        "none" -> Random.nextInt(1, 6)
        "sword" -> Random.nextInt(7, 19)
        else -> 0
    }
    println("вы нанесли $damage урона по $targetId")
    ...
    addScene {
        ...
        addPanelSurface {
            ...
            Column {
                ...
                ////////////////////////// <1.>
                Button("Атаковать рукой") {
                    modifier.onClick{dealDamage(game.playerId.value, "enemy", "hand", bus)}
                }
                Button("Атаковать мечом") {
                    modifier.onClick{dealDamage(game.playerId.value, "enemy", "sword", bus)}
                }
                /////////////////////////// </1.>
                ...
                Button("Switch Player") {
                    modifier.onClick {
                        val newId = if (game.playerId.value == "player_1") "player_2" else "player_1"
                        game.playerId.value = newId

                        saves.loadProgress(newId, quests.questId)
                    }
                }

                Button("Get Sword") {
                    modifier.onClick{quests.completeStep(game.playerId.value, 0)}
                }
                /////////////////////////// </2.>

                /////////////////////////// <3.>
                Text("Сохранения игрока:") {
                    modifier.margin(bottom = sizes.smallGap)
                }

                val savesList = saves.listSavesForPlayer(game.playerId.use())

                savesList.forEach { file ->
                    Button(file.name) {
                        modifier.onClick {
                            val parts = file.name.removeSuffix(".save").split("_")
                            if (parts.size >= 2) {
                                val player = parts[0]
                                val questId = parts[1]
                                saves.loadProgress(player, questId)
                            }
                        }
                    }
                }
...
// 1. сделать нанесение крит урона случайным значением
// - например рука (1-5) меч(7-18)
// - в консоли выволдить нанесение случайный урон
// ++

// 2. "квест выполняется отдельно для каждого игрока"
// - нажми switch player btn -> стань player_2
//нажми get sword btn (step0) -> поргресс у player_2 станет 1
// вернись на player_1 -> у него прогресс должен быть свой (не смешаный)
// ++

// 3. кнопка с выбором сохранения
// - выводить список в виле кнопок всех сохранений что хранятся в файле saves конкретного игрока
// - при нажатии по любой кнопке сохранение - сохранение должно загрузится
// --